...
$(document).ready(function(){
 ...
 function fetch_data()
 {
  $.ajax({
   ...
   success:function(data)
   {
    ...
    html += '<td contenteditable id="check_box"></td>';
    ...
    for(var count=0; count < data.length; count++)
    {
     ...
     html += '<td contenteditable="false" class="column_name" data-column_name="check_box" data-id="'+data[count].id+'"><input type="checkbox" />'+data[count].check_box+'</td>';
 ...
 $(document).on('click', '#add', function(){
  ...
  var check_box = $('#check_box').text();
  if(first_name != '' && last_name != '' && check_box != '')
  {
   $.ajax({
    url:"{{ route('livetable.add_data') }}",
    method:"POST",
    data:{first_name:first_name, last_name:last_name, check_box:check_box, _token:_token},
    success:function(data)
    {
     $('#message').html(data);
     fetch_data();
 ...
 $(document).on('blur', '.column_name', function(){
    ...
    var column_value = $(this).find('input')[0].checked;
